...
app.post('/webhook/:token', (req, res) => {
  ...
  child_process.exec(`${dockerCommand} login -u "${username}" -p "${password}"`,
    (error, stdout, stderr) => {
      ...
      child_process.exec(`${dockerCommand} image pull ${image} && ${dockerCommand} service update ${service} --force --image=${image}`,
        (error, stdout, stderr) => {
        if (error) {
	  ...
	  console.log(`Old containers and images removed successfully.`)
	})
  })
})
